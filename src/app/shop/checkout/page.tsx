...
export default function CheckoutPage() {
  ...
  const totalAmount = cart.reduce((sum, item) => item.discount ? sum + (item.price - item.price * item.discount/100) * item.quantity : sum + item.price * item.quantity, 0);
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex justify-center items-center p-4">
      <div className="bg-white shadow-lg rounded-lg max-w-2xl w-full p-6">
        <h2 className="text-2xl font-semibold text-gray-900 text-center mb-4">Thanh Toán Đơn Hàng</h2>
          ...
          {cart.map((item,index) => (
            <div key={index} className="flex justify-between py-2 border-b last:border-none">
              <span className="font-medium text-gray-600">{item.name}<span
                className="font-bold whitespace-nowrap ml-1 mr-1 text-blue-600"> | x {item.quantity} |</span></span>
              <span className="font-bold whitespace-nowrap">{item.discount ? (item.price - item.price * item.discount/100).toLocaleString("vi-VN") : item.price.toLocaleString("vi-VN")} đ</span>
            </div>
          ))}
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          ...
          <div className="mb-4">
            ...
            <div className="space-y-2">
              ...
              <label className="flex items-center space-x-2 cursor-pointer">
                <input type="radio" value="affterGetItem" {...register("paymentMethod")} className="w-5 h-5 text-blue-500"/>
